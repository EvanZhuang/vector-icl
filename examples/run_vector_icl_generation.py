...
def parse_args():
    ...
    parser.add_argument("--task_type", choices=["sentiment_analysis", "summarization", "molecule", "translation"], default="classification", help="The type of task to use")
...
def main():
    ...
    if accelerator.is_main_process:
        if args.push_to_hub:
            ...
            os.makedirs(os.path.join(args.output_dir, "jsons"), exist_ok=True)
    ...
    linear_device = torch.device("cuda:{}".format(model_device_id+2))
    ...
    if args.task_type == "summarization":
        prefix = "("
        postfix = ")'s summarization is: "
    elif args.task_type == "molecule":
        prefix = "("
        postfix = ")'s molecule caption is: "
    ...
    for data_ctr, data in enumerate(eval_dataloader): #.select(random.sample(range(len(eval_dataset)), 100)):
        ...
        with torch.no_grad() and autocast(dtype=torch.bfloat16):
            prompt_embed, attn_masks, input_lens_lst = few_shot_embedding_icl_generation(model_module, tokenizer, few_shot_dataset=raw_datasets["train"], n_shot=args.n_shot, n_prompt=args.n_prompt, max_seq_length=args.max_seq_length,  embed_model=embed_model, projector=linear_projector, prefix=prefix, postfix=postfix)
        ...
        print("Model Prediction: ", model_pred, "Label: ", data["label"])
        ...
        if accelerator.is_main_process:
            pbar.update(1)
    accelerator.wait_for_everyone()
    eval_results = metric_bleu.compute()
    eval_results_rouge = metric_rouge.compute()
    eval_results.update(eval_results_rouge)
