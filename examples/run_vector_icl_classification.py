...
def main():
    ...
    if os.path.exists(os.path.join(args.output_dir, "{}_{}shot.json".format(args.dataset_name.split("/")[-1], args.n_shot))):
    ...
    if args.embed_model_name_or_path == "nvidia/NV-Embed-v1":
        ...
        in_features = 4096
        expansion_ratio = 1
    # put it in a wrapper, for easy use
    embed_model = EncoderWrapper(model=embed_model, model_name_or_path=args.embed_model_name_or_path, max_length=args.max_seq_length, batch_size=8)
    ...
    # now this is just for sst2, but we can generalize this to any dataset
    column_names = eval_dataset.column_names
    for column_name in column_names: 
        if column_name != "idx": break

    if column_name != "text":
        raw_datasets = raw_datasets.rename_column(column_name, "text")
        train_dataset = train_dataset.rename_column(column_name, "text")
        eval_dataset = eval_dataset.rename_column(column_name, "text")
        print("Renamed the column {} to 'text'".format(column_name))
    ...
    target_verbalizer = {}
    few_shot_verbalizer = {}
    # Constraint the allowed tokens
    allowed_tokens = [tokenizer.eos_token, " ",]
    for key in verbalizer.keys():
        few_shot_verbalizer[verbalizer[key]] = key
        allowed_tokens.append(key)
        target_ids = tokenizer.encode(key)
        target_ids = [t for t in target_ids if t not in tokenizer.all_special_ids and len(tokenizer.decode(t)) > 1]
        # only use the first token here! This is a simplification
        target_ids = target_ids[0]
        target_verbalizer[target_ids] = torch.tensor([verbalizer[key]])
        if accelerator.is_main_process: print("View the marker for class {}".format(verbalizer[key]), key)
    ...
    eval_dataloader = DataLoader(eval_dataset, batch_size=1, collate_fn=None, num_workers=8, shuffle=True)
    ...
    for data_ctr, data in enumerate(eval_dataloader):
        ...
        pre_indicator_embeds = model.get_input_embeddings()(tokenizer("(", return_tensors="pt", padding=False, truncation=False, add_special_tokens=False)['input_ids'].to(model.device))
        post_indicator_embeds = model.get_input_embeddings()(tokenizer(")'s sentiment is", return_tensors="pt", padding=False, truncation=False, add_special_tokens=False)['input_ids'].to(model.device))
        inputs_embeds = torch.cat([prompt_embed, pre_indicator_embeds, inputs_embeds, post_indicator_embeds], dim=1)

        outputs = model.generate(inputs_embeds=inputs_embeds, max_new_tokens=3, pad_token_id=tokenizer.eos_token_id, return_dict_in_generate=True, logits_processor=logits_processor, temperature=2e-1)

        model_pred = tokenizer.decode(outputs['sequences'][0], skip_special_tokens=True)

        model_pred_class = None
        max_likelihood = -1e8
        model_pred_pure = model_pred.replace(".", "").strip().lower()
        for key in verbalizer.keys():
            key_pure = key.replace(".", "").strip().lower()
            if key_pure in model_pred.lower():
                model_pred_class = torch.tensor([verbalizer[key]]).to(model.device)
                break
            elif model_pred_pure in key_pure:
                model_pred_class = torch.tensor([verbalizer[key]]).to(model.device)
                break

        if model_pred_class is None:
            outputs = model.forward(inputs_embeds=inputs_embeds, return_dict=True)
            for key in target_verbalizer.keys():
                likelihood_target = outputs['logits'][0][0,key].detach()
                if likelihood_target > max_likelihood:
    ...
    eval_results_f1 = metric_f1.compute(average="macro")
    eval_results.update(eval_results_f1)
    
    if accelerator.is_main_process:
        print(eval_results)
        with open(os.path.join(args.output_dir, "{}_{}shot.json".format(args.dataset_name.split("/")[-1], args.n_shot)), "w") as f:
