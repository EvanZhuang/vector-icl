...
from embedding_merge import few_shot_embedding_icl, few_shot_embedding_icl_generation, few_shot_embedding_icl_timeseries
...
def parse_args():
    ...
    parser.add_argument("--embed_model_name_or_path", type=str, default="amazon/chronos-t5-tiny", help="The name or path of the embedding model to use")
    parser.add_argument("--task_type", choices=["sentiment_analysis", "summarization", "molecule", "translation", "graph", "timeseries"], default="classification", help="The type of task to use")
...
def main():
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=1, collate_fn=transformers.default_data_collator, num_workers=64, shuffle=True)
    eval_dataloader = DataLoader(eval_dataset, batch_size=1, collate_fn=transformers.default_data_collator, num_workers=8, shuffle=True)
    ...
    prefix = "("
    postfix = ")'s class (positive, negative) is"
    breaking_text = "\n"
    ...
    for data_ctr, data in enumerate(eval_dataloader): #.select(random.sample(range(len(eval_dataset)), 100)):
        ...
        with torch.no_grad() and autocast(dtype=torch.bfloat16):
            if args.n_shot == 0:
                ...
                prompt_embed, attn_masks, input_lens_lst = few_shot_embedding_icl_timeseries(model_module, tokenizer, few_shot_dataset=train_dataloader, n_shot=args.n_shot, n_prompt=args.n_prompt, max_seq_length=args.max_seq_length,  embed_model=time_model, projector=linear_projector, prefix=prefix, postfix=postfix)
        
            inputs_embeds = time_model.transform(data['features'].to(torch.device("cpu")), batch_size=args.per_device_eval_batch_size).unsqueeze(1)
            inputs_embeds = linear_projector(inputs_embeds.to(linear_projector.device))
            ...
            seperator_embeds = model.get_input_embeddings()(tokenizer(breaking_text, return_tensors="pt", padding=False, truncation=False, add_special_tokens=False)['input_ids'].to(model.device))
            
            if prompt_embed is None:
                inputs_embeds = torch.cat([pre_indicator_embeds, inputs_embeds, post_indicator_embeds], dim=1)
            else:
                inputs_embeds = torch.cat([prompt_embed, seperator_embeds, pre_indicator_embeds, inputs_embeds, post_indicator_embeds], dim=1)
            outputs = model.generate(inputs_embeds=inputs_embeds, max_new_tokens=3, pad_token_id=tokenizer.eos_token_id, do_sample=True, temperature=1, num_beams=1, return_dict_in_generate=True)
